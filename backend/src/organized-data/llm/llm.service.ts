...
export class LLMService {
  ...
  async generateOutput(
    ...
    if (!this.availableModels.has(model)) {
      throw new Error(`Model ${model} is not avaible.`);
    }
  ...
  async generateRefineOutput(
    model: string,
    intialPromptTemplate: PromptTemplate,
    refinePromptTemplate: PromptTemplate,
    chainValues: ChainValues,
  ) {
    if (!this.availableModels.has(model)) {
      throw new Error(
        `Model ${model} is not avaible for organized data extraction`,
      );
    }

    const refineChain = loadQARefineChain(this.availableModels.get(model), {
      questionPrompt: intialPromptTemplate,
